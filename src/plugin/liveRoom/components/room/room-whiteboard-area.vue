...
<script>
import RoomWhiteboardNone from './room-whiteboard-none'
import RoomWhiteboardCanvastools from './room-whiteboard-canvastools'
...
export default {
  ...
  inject: ['zegoWhiteboardArea', 'zegoLiveRoom', 'thisParent'],
  components: {
    RoomWhiteboardNone,
  ...
  data() {
    return {
      ...
      roomAuth: this.zegoLiveRoom.$http.auth,
      role: this.zegoLiveRoom.$http.role
    }
  },
  computed: {
    isThumbnailsVisible() {
  ...
  mounted() {
    this.init()
    BUS.$on('endTeaching', this.endTeaching)
  },
  destroyed() {
    BUS.$off('endTeaching', this.endTeaching)
  },
  methods: {
    async init() {
      try {
        this.WBViewList = await this.zegoWhiteboardArea.getViewList()
        this.initCanvasWidthHieght()
    ...
    endTeaching() {
      if (this.zegoLiveRoom.$http.role == this.thisParent.liveRoomParams.ROLE_STUDENT) this.$refs.endDialog.show = true
    }
  }
}
</script>
